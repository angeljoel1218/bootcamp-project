...
@Service
public class TransactionFixedTermAccountServiceImpl implements TransactionFixedTermAccountService<FixedTermAccountDto> {
    @Autowired
    ...
    public Mono<TransactionDto> deposit(DepositDto depositDto) {
        return fixedTermAccountRepository.findByNumberAndTypeAccount(depositDto.getTargetAccount(),TypeAccount.FIXED_TERM_ACCOUNT)
                ...
                .flatMap(fixedTermAccount -> {
                    ...
                    return productClient.getProductAccount(fixedTermAccount.getProductId())
                            ...
                            .flatMap(productAccountDto-> transactionRepository.countByDateBetweenAndAccountIdAndTypeNot(DateUtil.getStartOfMonth(), DateUtil.getEndOfMonth(), fixedTermAccount.getId(), TypeTransaction.ENTRY_TRANSFER)
                                    .doOnNext(count -> {
                                    ...
                                    .flatMap(count -> {
                                        BigDecimal totalAmount = fixedTermAccount.getBalance().add(depositDto.getAmount()).subtract(operationDto.getCommission());
                                        ...
                                        return fixedTermAccountRepository.save(fixedTermAccount)
                                                .flatMap(ft -> {
                                                    operationDto.setAmount(depositDto.getAmount().subtract(operationDto.getCommission()));
                                                    return this.transferBalanceService.saveTransaction(operationDto, TypeTransaction.DEPOSIT, TypeAffectation.INCREASE, TypeAccount.FIXED_TERM_ACCOUNT);
                                                });
                                    }));
                });
    }
    ...
    public Mono<TransactionDto> withdraw(WithdrawDto withdrawDto) {
        return fixedTermAccountRepository.findByNumberAndTypeAccount(withdrawDto.getAccountNumber(), TypeAccount.FIXED_TERM_ACCOUNT)
                .switchIfEmpty(Mono.error(new TransactionException("Account number does not exist")))
                .doOnNext(fixedTermAccount -> {
                    if(fixedTermAccount.getDayOfOperation() != DateUtil.getCurrentDay()) {
                        throw new TransactionException("Date not allowed to perform operations");
                ...
                .flatMap(fixedTermAccount ->productClient.getProductAccount(fixedTermAccount.getProductId())
                                ...
                                .flatMap(productAccountDto -> {
                                    ...
                                    return transactionRepository.countByDateBetweenAndAccountIdAndTypeNot(DateUtil.getStartOfMonth(), DateUtil.getEndOfMonth(), fixedTermAccount.getId(), TypeTransaction.ENTRY_TRANSFER)
                                            .doOnNext(count -> {
                                                BigDecimal commission = BigDecimal.ZERO;
                                                if (productAccountDto.getMaxMovements() <= count) {
                                                    commission = commission.add(BigDecimal.valueOf(productAccountDto.getCommissionAmount()));
                                            ...
                                            .flatMap(count -> fixedTermAccountRepository.save(fixedTermAccount)
                                                                .flatMap(ft -> this.transferBalanceService.saveTransaction(operationDto, TypeTransaction.WITHDRAW, TypeAffectation.DECREMENT, TypeAccount.FIXED_TERM_ACCOUNT)));
                                }));
    }
    @Override
    public Mono<TransactionDto> wireTransfer(TransactionRequestDto transferDto) {
        return fixedTermAccountRepository.findByNumberAndTypeAccount(transferDto.getSourceAccount(), TypeAccount.FIXED_TERM_ACCOUNT)
                .switchIfEmpty(Mono.error(new TransactionException("Account number does not exist")))
                .doOnNext(fixedTermAccount -> {
                    if(fixedTermAccount.getDayOfOperation() != DateUtil.getCurrentDay()) {
                        throw new TransactionException("Date not allowed to perform operations");
                                ...
                                return transactionRepository.countByDateBetweenAndAccountIdAndTypeNot(DateUtil.getStartOfMonth(), DateUtil.getEndOfMonth(), fixedTermAccount.getId(), TypeTransaction.ENTRY_TRANSFER)
                                        ...
                                        .flatMap(count -> transferBalanceService.balanceToTargetAccount(transferDto)
                                                .flatMap(operationDto1 -> fixedTermAccountRepository.save(fixedTermAccount)
                                                        .flatMap(ft-> this.transferBalanceService.saveTransaction(operationDto, TypeTransaction.OUTPUT_TRANSFER, TypeAffectation.DECREMENT, TypeAccount.FIXED_TERM_ACCOUNT))
                                                ));
                            });
                });
    }
