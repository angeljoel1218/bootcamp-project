...
public class TransactionCurrentAccountServiceImpl implements TransactionCurrentAccountService<CurrentAccountDto> {
  ...
  public Mono<TransactionDto> deposit(DepositDto depositDto) {
    return currentAccountRepository.findByNumberAndTypeAccount(
      depositDto.getTargetAccount(), TypeAccount.CURRENT_ACCOUNT)
                        ...
                        .flatMap(productDto ->
                          this.transferBalanceService.balanceToTargetAccount(transferDto)
                            .flatMap(txn -> {
                              ...
                              return currentAccountRepository.save(currentAccount)
                                  .flatMap(rs ->
                                    this.transferBalanceService.saveTransaction(operationDto,
    ...
    return currentAccountRepository.save(currentAccount)
        .flatMap(ca -> {
          ...
          return this.transferBalanceService.saveTransaction(operationDto,
            TypeTransaction.WITHDRAW, TypeAffectation.DECREMENT, TypeAccount.CURRENT_ACCOUNT);
        });
  }
}
