...
public class CardService {
  ...
  public Mono<CardDto> findCardByNumberAndCvv(String number, String cvv) {
    return reactiveCircuitBreakerFactory.create("${circuitbreaker.instances.name}")
                .run(cardClient.findCardByNumberAndCvv(number, cvv), throwable ->
                  Mono.error(new ServiceUnavailableException("The service is not available, try in a few minutes please")));
  }
