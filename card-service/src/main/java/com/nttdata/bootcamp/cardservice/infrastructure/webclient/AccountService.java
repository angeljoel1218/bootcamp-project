    ...
    public Flux<AccountDto> findByHolderId(String holderId) {
        return reactiveCircuitBreakerFactory.create("${circuitbreaker.instances.name}")
                .run(webClientBuilder
                        ...
                        .retrieve()
                        .bodyToFlux(AccountDto.class), throwable -> Flux.error(new ServiceUnavailableException("The service is not available, try in a few minutes please")));
    }
    ...
    public Mono<AccountDto> findByNumber(String number) {
        return reactiveCircuitBreakerFactory.create("${circuitbreaker.instances.name}")
                .run(webClientBuilder
                        ...
                        .uri("account/" + number)
                        .retrieve()
                        .bodyToMono(AccountDto.class), throwable -> Mono.error(new ServiceUnavailableException("The service is not available, try in a few minutes please")));
    }
