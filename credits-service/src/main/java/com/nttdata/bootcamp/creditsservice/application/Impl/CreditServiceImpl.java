...
public class CreditServiceImpl implements CreditService {
  ...
  private Mono<Credit>  createCreditDuesByCredit(Credit credit){
    ...
    while (numberDues <= numberTotalOfDues) {
      ...
      BigDecimal duesAmount = totalAmount
        .divide(BigDecimal.valueOf(numberTotalOfDues), 4, RoundingMode.HALF_UP);

      BigDecimal interestAmount = totalInterest
        .divide(BigDecimal.valueOf(numberTotalOfDues), 4, RoundingMode.HALF_UP);
    ...
    return creditDuesRepository.findByIdCreditAndNroDues(creditDuesDto.getIdCredit(),
      ...
      .flatMap(t->{
        if (creditDuesDto.getAmount().compareTo(t.getTotalAmount()) != 0){
          ...
          return Mono.error(new CreditException(
            String.format("The amount of the due must be %s",
              t.getTotalAmount().toString())));
        }
        if (t.getStatus().equals(CreditStatus.PAYED)){
          return Mono.error(new CreditException(
            String.format("The amount of the due have done paid at %s", t.getPaidDate())));
        }
