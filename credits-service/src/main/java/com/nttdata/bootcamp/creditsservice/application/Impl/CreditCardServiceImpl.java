...
public class CreditCardServiceImpl implements CreditCardService {
  ...
  @Autowired
  private TransactionCreditCardRepository transactionRepository;
  ...
  public Mono<CreditCard> create(CreditCard creditCard) {
    ...
    }).switchIfEmpty(Mono.error(new InterruptedException("Customer not found")));
  }
  ...
  public Mono<String> payment(TransactionCreditCard transactionCredit) {
    return creditCardRepository.findById(transactionCredit.getIdCredit()).flatMap(credit -> {
      BigDecimal totalUse = credit.getAmountUsed().subtract(transactionCredit.getAmount());
      ...
      return creditCardRepository.save(credit)
        .flatMap(updateCredit -> Mono.just(transactionCredit).flatMap(transactionRepository::insert)
  ...
  private Mono<CreditCardMonthly> payMonthlyStatus(CreditCard credit,
                                                   TransactionCreditCard trans) {

    return monthlyRepository.findFirstByIdCreditCardAndStatusOrderByEndDateAsc(trans.getIdCredit(),
        CreditStatus.PENDING)

      .filter(mt -> trans.getAmount().compareTo(mt.getAmount()) > 0
        && trans.getDate().after(mt.getEndDate()))
      .flatMap(pendingMonthly -> {

        BigDecimal rest = trans.getAmount().subtract(pendingMonthly.getAmount());
        pendingMonthly.setPaidDate(new Date());
        pendingMonthly.setStatus(CreditStatus.PAYED);

        if (rest.compareTo(BigDecimal.ZERO) > 0) {
          trans.setAmount(rest);
          payMonthlyStatus(credit, trans);
        }
        return  monthlyRepository.save(pendingMonthly);
      })
      .switchIfEmpty(registerCreditCardMonthly(trans, credit));
  }
    ...
    return monthlyRepository.findFirstByIdCreditCardOrderByEndDateDesc(transaction.getIdCredit())
     ...
     .flatMap(dateStart -> {
       ...
       dateOfEnd = dateOfEnd.before(dateStart)
         ? DateUtils.addMonths(dateOfEnd, BigDecimal.ONE.intValue()) : dateOfEnd;

       dateOfPay = dateOfPay.before(dateOfEnd)
         ? DateUtils.addMonths(dateOfPay, BigDecimal.ONE.intValue()) : dateOfPay;
  ...
  private BigDecimal affectationOperationMonthly(TypeTransaction typeTransaction) {
    return Objects.equals(typeTransaction, TypeTransaction.CHARGE) ? BigDecimal.ONE
  ...
  public Mono<Boolean> findIsCustomerHaveDebs(String idCustomer) {
    return  creditCardRepository.findByIdCustomer(idCustomer)
      .flatMap(list -> monthlyRepository.findFirstByIdCreditCardAndStatusOrderByEndDateAsc(
        list.getId(),
